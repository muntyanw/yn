...
<div class="container mt-5 mb-5">
    ...
    <form action="{{ route('admin_report_update', ['id' => $report->id]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="month" name="month" value="{{ old('month', $report->month) }}" required>
            @error('month')
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="year" name="year" value="{{ old('year', $report->year) }}" required>
            @error('year')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="text" name="text" required>{{ old('text', $report->text) }}</textarea>
            @error('text')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mt-3">
            ...
            @foreach ($report->photos as $photo)
                <div class="mb-2 d-flex align-items-center">
                    <img src="{{ asset('storage/' . $photo->photo) }}" alt="{{ __('Current Photo') }}" class="img-thumbnail" width="150">
                    <a href="{{ asset('storage/' . $photo->photo) }}" target="_blank" class="ml-3">{{ __('View Image') }}</a>
                </div>
            @endforeach
